...
export async function GET(request: Request) {
  ...
  if (slug) {
    // Static directory (markdown) wins on slug collision — see /explore.
    const staticClub = findStaticClub(slug)
    if (staticClub) {
      title = staticClub.name
      subtitle = firstSentence(staticClub.description) || `Vibeclub on ${staticClub.platform}`
      rhythm = prettyPreset(staticClub.preset)
...
function firstSentence(text: string, max = 160): string {
  ...
  if (period > 30 && period < max) return cleaned.slice(0, period + 1)
