...
const linearOverdue = [
  {
    id: 'ARC-109',
    title: 'BOIP Trademark Filing',
    due: 'Apr 16',
    status: 'Backlog',
    priority: 'Urgent',
    action: 'Manual — file at boip.int',
  },
  {
    id: 'ARC-113',
    title: 'Beehiiv Newsletter Setup',
    due: 'Apr 16',
    status: 'Backlog',
    priority: 'High',
    action: 'Manual — create account',
  },
  {
    id: 'ARC-86',
    title: 'Rename @frankxeth on X',
    due: 'Apr 5',
    status: 'In Progress',
    priority: 'Urgent',
    action: 'Manual — X settings',
  },
  {
    id: 'ARC-88',
    title: 'Post ACOS v10 thread',
    due: 'Apr 5',
    status: 'In Progress',
    priority: 'Urgent',
    action: 'Manual — post from queue',
  },
  {
    id: 'ARC-110',
    title: 'frankx.ai/tools affiliate',
    due: 'Apr 15',
    status: 'Backlog',
    priority: 'Urgent',
    action: 'Build page',
...
const StatBox = ({ label, value, sub, color = COLORS.teal }) => (
  <GlassCard style={{ textAlign: 'center', flex: 1, minWidth: 140 }}>
    ...
    {sub && (
      <div style={{ fontSize: 11, color: COLORS.muted, marginTop: 2, opacity: 0.7 }}>{sub}</div>
    )}
  </GlassCard>
)
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div>
            ...
            >
              {sessions.map((s) => (
                <GlassCard
                  key={s.id}
                  style={{ borderLeft: `3px solid ${categoryColors[s.category]}` }}
                >
                  <div
                    style={{
                      display: 'flex',
            ...
            <GlassCard style={{ marginTop: 16 }}>
              <h3
                style={{ fontSize: 14, fontWeight: 600, margin: '0 0 12px', color: COLORS.muted }}
              >
          ...
          <div>
            <GlassCard style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 12, color: COLORS.muted, margin: '0 0 12px' }}>
                All 6 sessions now have corresponding Linear issues. ARC-101 rescoped for OSS pivot
                (deadline → Apr 30).
              </p>
              <div
                style={{
            ...
            <GlassCard>
              <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 4px', color: COLORS.red }}>
                Overdue Issues ({linearOverdue.length})
              </h3>
              <p style={{ fontSize: 12, color: COLORS.muted, margin: '0 0 12px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {linearOverdue.map((issue) => (
                  ...
                  >
                    <span
                      style={{ fontSize: 12, fontWeight: 600, color: COLORS.red, minWidth: 65 }}
                    >
                      {issue.id}
                    </span>
                    <div style={{ flex: 1 }}>
        ...
        {activeTab === 'gate' && (
          <div>
            <GlassCard style={{ marginBottom: 16 }}>
              ...
              >
                <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0, color: COLORS.gold }}>
                  Gate 0: First Flame
                </h3>
                <span style={{ fontSize: 12, color: COLORS.muted }}>
                  Deadline: April 30 (rescoped)
                </span>
              </div>
              <p style={{ fontSize: 13, color: COLORS.muted, margin: '0 0 16px' }}>
                Strategy pivot: OSS-first, free-first. Revenue deferred until quality is
                unambiguous.
              </p>
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.gold, marginBottom: 4 }}>
                  Overall Gate Progress
                </div>
                <ProgressBar pct={35} color={COLORS.gold} />
                <div style={{ fontSize: 12, color: COLORS.muted, marginTop: 4 }}>
                  35% — GenCreator go-live is the critical path
                </div>
              </div>
            </GlassCard>
            ...
            <GlassCard>
              ...
              {[
                {
                  n: 1,
                  text: 'GenCreator go-live: create repo, link Vercel, set env vars, run go-live.ps1',
                  urgency: 'high',
                },
                { n: 2, text: 'BOIP Trademark filing at boip.int (€514-757)', urgency: 'high' },
                {
                  n: 3,
                  text: 'Start arcanea-dashboard-template (biggest market gap)',
                  urgency: 'medium',
                },
                {
                  n: 4,
                  text: 'Create arcanea-templates meta-repo with deploy buttons',
                  urgency: 'medium',
                },
                ...
                >
                  <span
                    style={{
                      fontSize: 16,
                      fontWeight: 700,
                      color: a.urgency === 'high' ? COLORS.red : COLORS.teal,
                      minWidth: 20,
                    }}
                  >
                    {a.n}
          ...
          <div>
            <GlassCard style={{ marginBottom: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 4px' }}>
                Week-over-Week Recovery
              </h3>
              <p style={{ fontSize: 12, color: COLORS.muted, margin: '0 0 16px' }}>
                Last week collapsed to 6/30. This week (mid): 17/30. +11 point recovery.
              </p>
            ...
            <GlassCard style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, lineHeight: 1.7, color: 'rgba(255,255,255,0.7)' }}>
                <p style={{ margin: '0 0 12px' }}>
                  <strong style={{ color: COLORS.teal }}>Problem:</strong> Claude Code sessions
                  produce massive output that never reaches tracking surfaces. Work is invisible in
                  Linear and Notion while the codebase advances rapidly.
                </p>
                <p style={{ margin: '0 0 12px' }}>
                  <strong style={{ color: COLORS.gold }}>Root cause:</strong> No automated bridge
                  between git commits and Linear/Notion. The /handover skill captures wisdom but is
                  opt-in per session. Daily Pulse requires manual trigger.
                </p>
                <p style={{ margin: 0 }}>
                  <strong style={{ color: COLORS.green }}>Solution built tonight:</strong> This
                  Cowork session synced all 6 sessions to Linear (7 issues created/updated) and
                  Notion (Daily Pulse + Weekly Scorecard). The /handover skill now routes to
                  Starlight Vaults automatically. Next: scheduled daily sync task.
                </p>
              </div>
            </GlassCard>
            ...
            <GlassCard>
              <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 8px' }}>
                Execution vs Tracking (This Session)
              </h3>
        ...
        >
          <span style={{ fontSize: 11, color: COLORS.muted }}>
            Arcanea Ops Sync Dashboard · Built by Cowork ·{' '}
